...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,18,0)
static int _noop_pre(struct kprobe *p, struct pt_regs *regs) { return 0; }
...
static typeof(&shmem_zero_setup) shmem_zero_setup_func = NULL;

static int _find_kallsyms_lookup_name(void)
{
	int ret = -1;
	kp.pre_handler = _noop_pre;
	ret = register_kprobe(&kp);
	if (ret < 0) {
		pr_err("register_kprobe failed. err: %d\n", ret);
		return ret;
	}
...
static int _find_shmem_zero_setup(void)
{
	int ret = -1;

	ret = _find_kallsyms_lookup_name();
	if (ret < 0) {
		return ret;
	}

	shmem_zero_setup_func = (typeof(&shmem_zero_setup)) kallsyms_lookup_name_func("shmem_zero_setup");
	if (shmem_zero_setup_func == NULL) {
		pr_err("failed to get shmem_zero_setup.\n");
		return -1;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,18,0)
	if (_find_shmem_zero_setup() < 0) {
		pr_err("failed to find register_zero_setup");
		return -EINVAL;
	}
#endif
